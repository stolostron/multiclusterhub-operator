...
var _ = Describe("MultiClusterHub controller", func() {
	...
	Context("When updating Multiclusterhub status", func() {
		...
		It("Should allow cluster-backup to be optional", func() {
			By("Applying prereqs")
			ctx := context.Background()
			ApplyPrereqs(k8sClient)

			By("By creating a new Multiclusterhub with cluster-backup disabled")
			mch := resources.NoSearchMCH()
			Expect(k8sClient.Create(ctx, &mch)).Should(Succeed())
			Expect(k8sClient.Create(ctx, mchoDeployment)).Should(Succeed())
			...
			By("Ensuring cluster-backup namespace does not exist")
			Eventually(func() bool {
				backupNamespace := types.NamespacedName{
					Name: "cluster-backup",
				}
				namespace := corev1.Namespace{}
				err := k8sClient.Get(ctx, backupNamespace, &namespace)
				if err != nil && errors.IsNotFound(err) {
					return true
				}
				return false
			...
			By("Updating MCH to enable cluster-backup")
			Eventually(func() bool {
				err := k8sClient.Get(ctx, resources.MCHLookupKey, createdMCH)
				return err == nil
			}, timeout, interval).Should(BeTrue())
			createdMCH.Spec.EnableClusterBackup = true
			Expect(k8sClient.Update(ctx, createdMCH)).Should(Succeed())

			By("Ensuring cluster-backup is subscribed")
			Eventually(func() error {
				backupNamespace := types.NamespacedName{
					Name: "cluster-backup",
				}
				dnamespace := corev1.Namespace{}
				return k8sClient.Get(ctx, backupNamespace, &dnamespace)
			}, timeout, interval).Should(Succeed())

			By("Updating MCH to disable cluster-backup")
			Eventually(func() bool {
				err := k8sClient.Get(ctx, resources.MCHLookupKey, createdMCH)
				return err == nil
			}, timeout, interval).Should(BeTrue())
			createdMCH.Spec.EnableClusterBackup = false
			Expect(k8sClient.Update(ctx, createdMCH)).Should(Succeed())
