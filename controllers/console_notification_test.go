...
func TestMCEComplianceBannerText_Ahead(t *testing.T) {
	got := mceComplianceBannerText("2.12.0", "stable-2.11")
	expected := "WARNING: ACM in unexpected configuration: MCE 2.12.0 is ahead of the expected stable-2.11 channel."
	if got != expected {
...
func TestMCEComplianceBannerText_Behind(t *testing.T) {
	got := mceComplianceBannerText("2.10.0", "stable-2.11")
	expected := "WARNING: ACM in unexpected configuration: MCE 2.10.0 is behind the expected stable-2.11 channel."
	if got != expected {
		t.Errorf("mceComplianceBannerText() = %q, want %q", got, expected)
	}
}
...
func TestOCPComplianceBannerText(t *testing.T) {
	got := ocpComplianceBannerText("4.17.0", "4.18.0")
	expected := "WARNING: ACM in unexpected configuration: OCP 4.17.0 is below the minimum supported version 4.18.0."
	if got != expected {
...
func TestEnsureMCEComplianceBanner_NonCompliant(t *testing.T) {
	...
	compliance := &operatorsv1.MCEVersionComplianceStatus{
		...
		IsCompliant:     false,
		Message:         "MCE version 2.12.0 does not meet channel stable-2.11 requirements",
	}
	...
	expectedText := mceComplianceBannerText("2.12.0", "stable-2.11")
...
func TestEnsureMCEComplianceBanner_UpdatesExistingBanner(t *testing.T) {
	...
	existing := &consolev1.ConsoleNotification{
		...
		Spec: consolev1.ConsoleNotificationSpec{
			Text:            mceComplianceBannerText("2.12.0", "stable-2.11"),
			Location:        consolev1.BannerTop,
			BackgroundColor: bannerBackgroundColor,
			Color:           bannerTextColor,
		},
	...
	newCompliance := &operatorsv1.MCEVersionComplianceStatus{
		...
		IsCompliant:     false,
		Message:         "MCE version 2.13.0 does not meet channel stable-2.11 requirements",
	}
	...
	expectedText := mceComplianceBannerText("2.13.0", "stable-2.11")
	if notification.Spec.Text != expectedText {
...
// TestEnsureOCPComplianceBanner_BelowMinimum verifies that a banner is created
// when OCP is below MinimumOCPVersion (4.18.0 for ACM 2.16).
func TestEnsureOCPComplianceBanner_BelowMinimum(t *testing.T) {
	...
	// 4.17.0 is below MinimumOCPVersion (4.18.0)
	err := recon.ensureOCPComplianceBanner(ctx, hub, "4.17.0")
	if err != nil {
...
func TestEnsureOCPComplianceBanner_MeetsMinimum_RemovesBanner(t *testing.T) {
	...
	// 4.18.0 meets MinimumOCPVersion (4.18.0) — banner should be removed
...
func TestEnsureOCPComplianceBanner_UpdatesExistingBanner(t *testing.T) {
	...
	// Create banner with old version (4.16.x — below minimum)
	existing := &consolev1.ConsoleNotification{
		ObjectMeta: metav1.ObjectMeta{
			Name: ocpComplianceBannerName,
			Labels: map[string]string{
		...
		Spec: consolev1.ConsoleNotificationSpec{
			Text:            ocpComplianceBannerText("4.16.0", version.MinimumOCPVersion),
			Location:        consolev1.BannerTop,
			BackgroundColor: bannerBackgroundColor,
			Color:           bannerTextColor,
		},
	...
	// Update with a different OCP version still below minimum (4.17.x)
	err := recon.ensureOCPComplianceBanner(ctx, hub, "4.17.0")
	if err != nil {
		t.Fatalf("ensureOCPComplianceBanner() error = %v", err)
	}
