...
func (r *MultiClusterHubReconciler) cleanupMultiClusterEngine(log logr.Logger, m *operatorsv1.MultiClusterHub) error {
	...
	csv, err := r.GetCSVFromSubscription(multiclusterengine.Subscription(m, r.GetSubConfig()))
	...
	if err == nil {
		err = r.Client.Delete(ctx, multiclusterengine.Subscription(m, r.GetSubConfig()))
		if err != nil && !errors.IsNotFound(err) {
			return err
		}
