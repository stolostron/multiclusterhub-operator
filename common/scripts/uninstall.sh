...
oc get csv | grep "etcdoperator" | awk '{ print $1 }' | xargs oc delete csv --wait=false --ignore-not-found || true
...
oc get crd | grep "hive" | awk '{ print $1 }' | xargs oc delete crd --wait=false --ignore-not-found
oc get crd | grep "etcd.database.coreos.com" | awk '{ print $1 }' | xargs oc delete crd --wait=false --ignore-not-found
