...
DESIRED=$(cat hack/scripts/targetCRDs.txt)
CURRENT=$(cat hack/scripts/currentCRDs.txt)
...
if [[ $DESIRED == main ]] || [[ $DESIRED == master ]] || [[ $DESIRED == release* ]]; then
    ...
    echo -n $LATEST_SHA > hack/scripts/currentCRDs.txt
else
    # Must be a sha
    echo "Setting current SHA to desired SHA"
    echo -n $DESIRED > hack/scripts/currentCRDs.txt
fi
