...
func TestSetTargetCertKeyPairSecret(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			err = setTargetCertKeyPairSecret(c.targetSecret, c.hostNames, c.signer)
...
func newServingCertKeyPair(hostName, signerName string, validity time.Duration) (certData, keyData, caData []byte, err error) {
	...
	certKeyPair, err := signer.MakeServerCertForDuration(sets.Set[string](sets.NewString(hostName)), validity)
	if err != nil {
		return nil, nil, nil, err
	}
